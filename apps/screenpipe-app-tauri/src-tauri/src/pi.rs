...
/// litepipe: the upstream web-search extension talked to the hosted cloud
/// backend, which does not exist here. Remove it if a previous install left
/// one behind so the agent never tries to call it.
fn ensure_web_search_extension(
    ...
    if ext_path.exists() {
        ...
        info!("web-search extension removed (no hosted backend in litepipe)");
    }
...
pub struct PiProviderConfig {
    /// Provider type: "openai", "openai-chatgpt", "native-ollama", "anthropic", "custom"
    pub provider: String,
...
async fn build_models_json(
    ...
    // litepipe: no hosted provider is ever injected. Only the provider the
    // user configured (Ollama or bring-your-own-key) lands in models.json.
    let _ = user_token;
...
/// Lightweight model-name normalization (strip date suffixes). Kept for
/// legacy "screenpipe" provider entries that may exist in an old models.json;
/// all other providers pass through untouched.
...
error: InstallFailed extracting tarball"#;
    ...
    // litepipe: no hosted provider is ever injected into models.json.
    #[tokio::test]
    async fn test_build_models_json_default_is_empty() {
    ...
    #[tokio::test]
    async fn test_build_models_json_ignores_user_token() {
    ...
    #[tokio::test]
    async fn test_build_models_json_unknown_provider_adds_nothing() {

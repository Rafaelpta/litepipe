...
async fn main() {
    ...
    // litepipe: no error reporting. Sentry is never initialized, so the panic
    // hook, the tauri plugin registration, and the tracing layer below (all
    // Option-gated on this guard) are inert and nothing leaves the machine.
        ...
        .setup(move |app| {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                let app_submenu = app_submenu_builder
                    .item(&PredefinedMenuItem::quit(app, Some("Quit litepipe"))?)
                    .build()?;
            ...
            // litepipe: the analytics manager is never started. Consumers use
            // try_state::<Arc<AnalyticsManager>>() and no-op when it is absent.
            ...
            // litepipe: background non-capture schedulers/pollers removed to keep the
            // app lightweight. These ran AI and integration work on a timer in the
            // background, all orthogonal to capture: the AI suggestions scheduler,
            // the pipe suggestions scheduler, the calendar events publisher, the
            // ICS calendar poller, and the Google Calendar publisher. Their modules
            // and managed state stay registered (nothing else breaks); only the
            // auto-start on boot is gone. `suggestions_state` stays bound above for
            // its command handlers.
            let _ = &suggestions_state;

            // litepipe: cloud sync and cloud archive auto-start removed (background
            // network work, not capture; the app is local-first). Their modules
            // and managed state stay registered; only the auto-start on boot is gone.

            // Auto-start local data retention (litepipe ships this ON by default)
            let app_handle_clone = app_handle.clone();
            tauri::async_runtime::spawn(async move {

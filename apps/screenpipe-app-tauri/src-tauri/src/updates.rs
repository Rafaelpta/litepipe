        ...
        if let Ok(Some(update)) = check_result {
            ...
            *self.pending_update.lock().await = Some(PendingUpdateSnapshot {
            ...
            if let Some(ref item) = self.update_menu_item {
                ...
                item.set_text("Downloading latest version of litepipe")?;
            }
                        ...
                        || err_str.contains("Forbidden")
                    {
                        ...
                        std::thread::spawn(move || {
                            let _ = app_notif
                                .notification()
                                .builder()
                                .title("litepipe update available")
                    ...
                    *self.pending_update.lock().await = None;
                    ...
                    std::thread::spawn(move || {
                        let _ = app_notif
                            .notification()
                            .builder()
                            .title("litepipe update failed")
                            .body(format!(
                                "v{} couldn't download — open litepipe to retry",
                                version_str
                            ))
                            .show();
                    });
...
fn check_whats_new(app: &tauri::AppHandle) {
    ...
    tokio::spawn(async move {
        ...
        let body = if release_notes.is_empty() {
            format!(
                "litepipe updated to **v{}**! check the [changelog](https://screenpi.pe/changelog) for details.",
                current_version
            )
        } else {
        ...
        match client
            ...
            .json(&serde_json::json!({
                ...
                "type": "app-update",
                "title": format!("🎉 litepipe v{} — what's new", current_version),
                "body": body,
            }))
            .send()
            .await
...
pub fn start_update_check(
    ...
    // litepipe: no automatic update checks. The updater endpoints are empty in
    // every build config and neither the boot check nor the periodic check is
    // spawned, so the app never contacts an update server. The manager is still
    // created because the tray menu and commands read it as managed state.

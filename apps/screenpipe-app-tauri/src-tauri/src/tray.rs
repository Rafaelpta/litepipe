...
fn create_dynamic_menu(
    ...
    if !data.onboarding_completed && !data.app_ui_hidden {
        menu_builder = menu_builder
            ...
            .item(&PredefinedMenuItem::separator(app)?)
            .item(&MenuItemBuilder::with_id("quit", "Quit litepipe").build(app)?);
    ...
    if !data.app_ui_hidden {
        menu_builder = menu_builder
            .item(&MenuItemBuilder::with_id("open_app", "Open litepipe").build(app)?)
            .item(&PredefinedMenuItem::separator(app)?);
    }
    ...
    let version_text = match (is_beta, is_enterprise) {
        (_, true) => format!("litepipe v{} (Enterprise)", app.package_info().version),
        (true, false) => format!("litepipe v{} (Beta)", app.package_info().version),
        (false, false) => format!("litepipe v{}", app.package_info().version),
    };
    menu_builder = menu_builder.item(
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("quit", "Quit litepipe")
    ...
    match event.id().as_ref() {
        ...
        id if id.starts_with("pause_") => {
            ...
            let handle = tauri::async_runtime::spawn(async move {
                ...
                send_notify("Recording resumed", "litepipe is recording again.");
            });
            *PAUSE_TIMER.lock().unwrap_or_else(|e| e.into_inner()) = Some(PauseTimer {
            ...
            send_notify(
                "Recording paused",
                format!("litepipe will auto-resume in {}.", pretty),
            );
...
async fn update_menu_if_needed(
    ...
    let tooltip: String = if has_perm_issue {
        "litepipe — ⚠️ permissions needed".to_string()
    } else if effective_status == RecordingStatus::Paused {
        match pause_remaining() {
            Some(d) => format!("litepipe — paused, resumes in {}", format_remaining(d)),
            None => "litepipe — paused".to_string(),
        }
    } else {
        "litepipe".to_string()
    };

...
// Pipe creation context — mirrors the screenpipe-pipe-creator SKILL.md
const PIPE_CONTEXT = `create a litepipe pipe that does the following.

## what is a pipe?

a pipe is a scheduled AI agent defined as a single markdown file: ~/.screenpipe/pipes/{name}/pipe.md
every N minutes, litepipe runs a coding agent (like pi or claude-code) with the pipe's prompt.
...
before execution, litepipe prepends a context header to the prompt with:
- time range (start/end timestamps based on the schedule interval)
- current date
- user's timezone
- litepipe API base URL
- output directory
...
export function SchedulePromptDialog({
  ...
  const handleCreate = () => {
    ...
    // Build a complete pipe creation message with full context
    const userRequest = [
      `Create a pipe called "${name}" with schedule: ${schedule}`,
      ``,
      `The pipe should run this prompt against my litepipe data:`,
      ``,

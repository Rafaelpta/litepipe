...
export default function GeneralSettings() {
  ...
  const handleAutoStartChange = async (checked: boolean) => {
    ...
    try {
      ...
      toast({
        ...
        description: checked
          ? "litepipe will launch when your computer starts"
          : "litepipe won't launch at startup",
      });
    } catch (e: any) {
      handleSettingsChange({ autoStartEnabled: !checked });
      toast({
  ...
  const handleOpenVersions = async () => {
    // litepipe: releases live on GitHub, not a vendor account page.
    const url = "https://github.com/Rafaelpta/litepipe/releases";
    try {
      await openUrl(url);
    } catch (e: any) {
      ...
      toast({
        ...
        description: e?.toString() || "check your browser for releases",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <div className="space-y-5" data-testid="section-settings-general">
      ...
      <div className="space-y-2">
        ...
        {/* litepipe: enterprise-managed update card and the store-specific
            "Auto-Update Pipes" card removed. */}
        <Card className="border-border bg-card">
          <CardContent className="px-3 py-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Use litepipe Cloud for smarter suggestions
                  </p>
                  <p className="text-[10px] text-muted-foreground/60 mt-0.5">
                    sends recent activity to litepipe cloud for processing. we don&apos;t store your data (zero retention).
                  </p>
                </div>
              </div>
              <Switch

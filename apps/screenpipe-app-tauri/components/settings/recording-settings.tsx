...
// litepipe: only the Deepgram key check remains — the hosted cloud engine and
// its login/subscription fallbacks are gone.
type AudioEngineFallbackReason = "missingDeepgramKey";
  ...
  return (
    <Card className="border-border bg-card">
      <CardContent className="px-3 py-2.5">
        ...
        >
          <Input name="vocab-word" placeholder="e.g. litepipe" className="h-7 text-xs flex-1" spellCheck={false} autoCorrect="off" autoCapitalize="off" />
...
export function RecordingSettings() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      // litepipe: telemetry is permanently disabled. There is no re-enable path;
      // PostHog is never initialized and Sentry is never started.
    ...
    if (resWriteExcl.status === "error") throw new Error(resWriteExcl.error);
      ...
      toast({
        ...
        description: needsServerRestart
          ? "litepipe server restarted with new settings"
  ...
  const handleAudioTranscriptionModelChange = async (
    ...
    // litepipe: guard against stale persisted values — the hosted cloud engine
    // is no longer selectable (its option was removed from the UI).
...
Your screen is a pipe. Everything you see, hear, and type flows through it. litepipe just makes sure nothing valuable leaks away.`;
          ...
          <CardContent className="px-3 py-2.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                  Your name
                  <HelpTooltip text="Your name in transcripts. Click 'train' and speak for 30 seconds to teach litepipe your voice — it will recognize you across all devices using voice matching." />
                </h3>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2.5">
                ...
                <div>
                  <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                    Live meeting notes
                    <HelpTooltip text="Streams only the active meeting into the live note. This is separate from background 24/7 recording and can use your selected transcription engine or a direct provider." />
                  </h3>
            ...
            {(settings.meetingLiveTranscriptionEnabled ?? true) && (
              <div className="mt-2.5 ml-[26px] flex flex-col gap-2">
                ...
                {(settings.meetingLiveTranscriptionProvider ?? "selected-engine") === "selected-engine" &&
                  settings.audioTranscriptionEngine === "disabled" && (
                  <p className="text-xs text-muted-foreground">
                    Pick an audio transcription engine above, or choose the direct Deepgram live provider.
                  </p>
                )}
              </div>
            )}
            <div className="mt-2.5 pt-2.5 border-t border-border/50 flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                  ...
                  <HelpTooltip text="When the meeting stops, litepipe appends what you typed (and the files you edited) during the meeting to the meeting note. Turn off to keep notes clean." />
                </h3>
      ...
      <Dialog open={voiceTraining.dialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogDescription className="text-xs text-muted-foreground">
            speak naturally at your normal pace — this helps litepipe learn your voice
          </DialogDescription>

...
import { IntegrationIcon, INTEGRATION_ICON_KEYS } from "@/components/chat/integration-icon";
import {
  formatSteerShortcut,
...
export function StandaloneChat({
  ...
  // litepipe: no hosted provider, so there is no login gate — every provider
  // is local or bring-your-own-key.
  ...
  useEffect(() => {
    ...
    const handlePiEventData = (data: any) => {
        ...
        if (
          ...
          if (piMessageIdRef.current) {
            ...
            if (quotaErrorType === "daily" || quotaErrorType === "rate") {
              const content = quotaErrorType === "daily"
                ? buildDailyLimitMessage(fullError)
                : buildRateLimitMessage(fullError);
              setMessages((prev) =>
                prev.map((m) => m.id === msgId ? { ...m, content } : m)
              );
          ...
          // Rate-limit (429) auto-retry — honor the provider's reset_in hint and
          // re-send the same prompt. BYOK providers cap requests/minute; a single
          // agentic run can trip that, after which a short wait clears the
          // budget. Without this the turn dies silently (e.g. pipe creation
          // stalls mid-skill).
          if (
    ...
    const setup = async () => {
      ...
      unlistenLog = await listen<string>("pi_log", (event) => {
        ...
        if (line.includes("403")) {
          ...
          if (msgId) {
            setMessages((prev) =>
              prev.map((m) => m.id === msgId ? { ...m, content: "Access denied by the provider (403) — check your API key or try a different model in your AI preset." } : m)
            );
          }
    ...
    // litepipe: the "pi-reauth" listener (session restart on deeplink login)
    // existed only for hosted-account re-auth and was removed with it.

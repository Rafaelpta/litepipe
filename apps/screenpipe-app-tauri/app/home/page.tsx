...
// litepipe: "pipes" (store) and "connections" sections removed.
const ALL_SECTIONS = [
...
function HomeContent() {
  ...
  // the user is on Timeline / Meetings / Settings — without it, status would
  ...
  // panel. Without this, clicking a chat from the Timeline /
  ...
  // "Try in Chat" — switch to chat view so the pre-filled prompt
  // (set by standalone-chat.tsx) becomes visible.
  useEffect(() => {
  ...
  // Top-level nav items
  // litepipe: "Pipes" (store) and "Connections" entries removed.
  const mainSections = [
      ...
      <div className="h-screen flex min-h-0">
          ...
          <AppSidebar collapsed={sidebarCollapsed} className="pl-4">
            ...
            <div className="p-2 flex-1 flex flex-col min-h-0">
              ...
              {/* Bottom items */}
              {/* litepipe: team invite and referral promo entries removed. */}
          ...
          <div className={cn("flex-1 flex flex-col h-full bg-background min-h-0 relative", isTranslucent ? "rounded-none" : "rounded-tr-lg")}>
            {/* ALWAYS-MOUNTED chat layer.
                ...
                state survives navigation to Timeline / Meetings / Settings.
